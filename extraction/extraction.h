auto GenPart_MuonIdx(ROOT::VecOps::RVec<float> &gen_pt, ROOT::VecOps::RVec<int> &muon_part_idx) {

  auto size_outer = gen_pt.size();
  ROOT::VecOps::RVec<int> mask(size_outer);
  auto size_inner = muon_part_idx.size();


  for (auto i = 0; i < size_outer; i++) {
    mask[i] = 0;
    for (auto j = 0; j < size_inner; j++) {
      if (muon_part_idx[j] == i) {
        mask[i] = 1;
      } 
    }
  }
...
auto MDeltaPhi(ROOT::VecOps::RVec<float> &Phi1, ROOT::VecOps::RVec<float> &Phi2) {
  auto size = Phi1.size();
...
auto Mclosest_jet_dr(ROOT::VecOps::RVec<float> &etaj, ROOT::VecOps::RVec<float> &phij, ROOT::VecOps::RVec<float> &etam, ROOT::VecOps::RVec<float> &phim) {
...
auto Mclosest_jet_mass(ROOT::VecOps::RVec<float> &etaj, ROOT::VecOps::RVec<float> &phij, ROOT::VecOps::RVec<float> &etam, ROOT::VecOps::RVec<float> &phim, ROOT::VecOps::RVec<float> &massj) {
...
auto Mclosest_jet_pt(ROOT::VecOps::RVec<float> &etaj, ROOT::VecOps::RVec<float> &phij, ROOT::VecOps::RVec<float> &etam, ROOT::VecOps::RVec<float> &phim, ROOT::VecOps::RVec<float> &ptj) {
...
auto Mclosest_jet_deta(ROOT::VecOps::RVec<float> &etaj, ROOT::VecOps::RVec<float> &phij, ROOT::VecOps::RVec<float> &etam, ROOT::VecOps::RVec<float> &phim) {
...
auto Mclosest_jet_dphi(ROOT::VecOps::RVec<float> &etaj, ROOT::VecOps::RVec<float> &phij, ROOT::VecOps::RVec<float> &etam, ROOT::VecOps::RVec<float> &phim) {
...
auto muon_clean_genjet_mask(ROOT::VecOps::RVec<float> &jet_pt, ROOT::VecOps::RVec<float> &jet_eta, ROOT::VecOps::RVec<float> &jet_phi, ROOT::VecOps::RVec<float> &lep_pt, ROOT::VecOps::RVec<float> &lep_eta, ROOT::VecOps::RVec<float> &lep_phi) {
...
auto muon_closest_jet_flavour_encoder(ROOT::VecOps::RVec<float> &etaj, ROOT::VecOps::RVec<float> &phij, ROOT::VecOps::RVec<float> &etae, ROOT::VecOps::RVec<float> &phie, \
  ROOT::VecOps::RVec<int> &fj, ROOT::VecOps::RVec<int> flavours) {

  //General function to encode the hadron and parton flavour of the closest Jet object. To be used for flavour one-hot encoding for training.
...
// auto GenPart_MuonIdx_totNum(ROOT::VecOps::RVec<float> &gen_pt, ROOT::VecOps::RVec<int> &muon_part_idx) {

//   auto size_outer = gen_pt.size();
//   auto size_inner = muon_part_idx.size();

//   ROOT::VecOps::RVec<int> counter(size_outer);
